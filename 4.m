...
% Клас 1
mu1 = [2, 3];           % середнє
sigma1 = [1 0.5; 0.5 1]; % коваріація
X1 = mvnrnd(mu1, sigma1, 50);

% Клас 2
mu2 = [5, 6];
sigma2 = [1 -0.8; -0.8 1.5];
X2 = mvnrnd(mu2, sigma2, 50);

X = [X1; X2];          % об'єднуємо дані
...
[x1Grid, x2Grid] = meshgrid(min(X(:,1))-1:0.1:max(X(:,1))+1, min(X(:,2))-1:0.1:max(X(:,2))+1);
xGrid = [x1Grid(:), x2Grid(:)];

lda_pred = predict(lda_model, xGrid);
lda_pred = reshape(lda_pred, size(x1Grid));

%% QDA
qda_model = fitcdiscr(X, y, 'DiscrimType', 'quadratic');
qda_pred = predict(qda_model, xGrid);
qda_pred = reshape(qda_pred, size(x1Grid));
